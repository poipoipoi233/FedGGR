import torch
import torch.nn.functional as F
from torch.nn import ModuleList
from torch_geometric.data import Data
from torch_geometric.nn import SAGEConv
import torch.nn as nn
from federatedscope.register import register_model


class FedGSL_sage(torch.nn.Module):
    def __init__(self,
                 in_channels,
                 out_channels,
                 hidden=64,
                 max_depth=2,
                 dropout=.0,
                 loc_gnn_outsize=64,
                 glob_gnn_outsize=64):
        super(FedGSL_sage, self).__init__()
        self.convs = ModuleList()
        self.lins = ModuleList()

        self.num_layers = max_depth

        self.convs.append(SAGEConv(in_channels, hidden))
        for _ in range(self.num_layers - 2):
            self.convs.append(SAGEConv(hidden, hidden))
        self.convs.append(SAGEConv(hidden, loc_gnn_outsize))

        if max_depth == 1:
            self.convs.append(GATConv(in_channels, loc_gnn_outsize))
        else:
            for i in range(max_depth):
                if i == 0:
                    self.convs.append(GATConv(in_channels, hidden))
                elif (i + 1) == max_depth:
                    self.convs.append(GATConv(hidden, loc_gnn_outsize))
                else:
                    self.convs.append(GATConv(hidden, hidden))
        self.dropout = dropout


        self.lin1 = nn.Linear(loc_gnn_outsize + glob_gnn_outsize, out_channels)
        # self.lins.append(nn.Linear(loc_gnn_outsize + glob_gnn_outsize, 64))
        # self.lins.append(nn.Linear(64, out_channels))
    def reset_parameters(self):
        for m in self.convs:
            m.reset_parameters()

    def forward(self, batch_x, edge_index, glob_emb):
        x = batch_x
        for i, conv in enumerate(self.convs):
            x = conv(x, edge_index)
            if (i + 1) == len(self.convs):
                break
            x = F.relu(F.dropout(x, p=self.dropout, training=self.training))
        # loc_emb = x
        emb = torch.cat((x, glob_emb), dim=1)  # 直接拼接


        # for i, lin in enumerate(self.lins[:-1]):
        #     x = lin(emb)
        #     x = F.leaky_relu(x)
        #     x = F.dropout(x, p=self.dropout, training=self.training)
        # res = self.lins[-1](x)
        # return res
        res = self.lin1(emb)
        return res  # , loc_emb


# Instantiate your model class with config and data
def ModelBuilder(model_config, local_data):
    model = FedGSL_GAT(in_channels=local_data[0][1],
                       out_channels=model_config.out_channels,
                       hidden=model_config.fedgsl.loc_gnn_hid,
                       max_depth=model_config.fedgsl.gnn_layer,
                       dropout=model_config.dropout,
                       loc_gnn_outsize=model_config.fedgsl.loc_gnn_outsize,
                       glob_gnn_outsize=model_config.fedgsl.glob_gnn_outsize
                       )
    return model


def call_my_net(model_config, local_data):
    if model_config.type == "gnn_fedgsl_sage":
        model = ModelBuilder(model_config, local_data)
        return model


register_model("gnn_fedgsl_sage", call_my_net)
