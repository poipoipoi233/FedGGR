...
class MaskedGCNConv(MessagePassing):
    ...
    def __init__(self, in_channels: int, out_channels: int,
        ...
        self.mask_rank = args.mask_rank #初始-1
        self.mask_drop = args.mask_drop #初始Flase
        self.mask_drop_ratio = args.mask_drop_ratio #初始0.5
        self.mask_noise = args.mask_noise #初始False

        self._cached_edge_index = None
        self._cached_adj_t = None

        #####################################################
        self.lin = MaksedGCNLinear(in_channels, out_channels, bias=False, l1=l1,
        ...
        if self.mask_rank == -1:
            ...
            self.pre = torch.nn.Parameter(torch.ones((self.mask_rank,self.d_i), requires_grad=True, dtype=torch.float32))
            self.pos = torch.nn.Parameter(torch.ones((self.d_o,self.mask_rank), requires_grad=True, dtype=torch.float32))
            self.mask = torch.ones((self.d_o,self.d_i), requires_grad=True, dtype=torch.float32)    # not a member of state_dict

        if not self.mask_one_init: #是否全一初始化？
            if self.mask_rank == -1:
                torch.nn.init.xavier_uniform_(self.mask)
            else:
                torch.nn.init.xavier_uniform_(self.pre)
    ...
    def set_mask(self):
        if self.mask_rank != -1:
            self.mask = torch.matmul(self.pos, self.pre) / self.mask_rank
        return self.mask
    ...
    def forward(self, x: Tensor, edge_index: Adj,
        ...
        if self.mask_noise and self.training:
            curr_mask = curr_mask + torch.empty(curr_mask.size(), device=curr_mask.device).normal_(mean=0, std=self.l1)

        if self.mask_drop:
            curr_mask = F.dropout(curr_mask, p=self.mask_drop_ratio, training=self.training)
...
#####################################################
# Source Code From TORCH_GEOMETRIC.NN.DENSE.LINEAR  #
#####################################################
import copy
import math
...
class MaskedLinear(torch.nn.Module):
    def __init__(self, d_i, d_o, l1=1e-3, args=None):
        ...
        self.mask_rank = args.mask_rank
        self.mask_drop = args.mask_drop
        self.mask_drop_ratio = args.mask_drop_ratio
        self.mask_noise = args.mask_noise
