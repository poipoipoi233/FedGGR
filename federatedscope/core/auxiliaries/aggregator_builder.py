...
def get_aggregator(method, model=None, device=None, online=False, config=None):
    ...
    elif method.lower() == "fedpub":
        #For FEDPUB
        from federatedscope.FedGSL.baseline.FEDPUB.FEDPUB_Aggregator import FEDPUB_Aggregator
        return FEDPUB_Aggregator(config=config, model=model, device=device)
